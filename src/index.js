...
class JSONInput extends Component {
  ...
  render() {
    ...
    /*React.createElement("svg", {
      ...
      style: _objectSpread({
        display: 'flex',
        overflow: 'none',
        height: hasError ? 'calc(100% - 30px)' : '100%',
        width: '',
        margin: 0,
        resize: 'none',
  ...
  newSpan(i, token, depth) {
    ...
    return '<span type="' + type + '" vOrigin="' + string + '" depth="' + depth + '" style="color:' + color + '">' + string + '</span>';
  }
  ...
  showValue() {
    const valueDoesNotExist = !('value' in this.props);
    if (valueDoesNotExist) return;
    const {
      value
    } = this.props;
    const valueHasEmptyValues = [undefined, null].indexOf(value) > -1;
    if (valueHasEmptyValues) return;
    const {
      prevValue,
      jsObject
    } = this.state;
    ...
    const valueDataType = getType(value);
    const unexpectedDataType = ['object', 'array'].indexOf(valueDataType) === -1;
    if (unexpectedDataType) err.throwError('showValue', 'value', 'either an object or an array');
    const sameValueValues = identical(value, prevValue); // Component will always re-render when new value vOrigin is any different from previous value vOrigin.

    let componentShouldUpdate = !sameValueValues;

    if (!componentShouldUpdate) {
      if (resetConfiguration) {
        /**
         * If 'reset' property is set true or is truthy,
         * any difference between value and current vOrigin
         * should trigger component re-render
         */
        if (jsObject !== undefined) componentShouldUpdate = !identical(value, jsObject);
      }
    }

    if (!componentShouldUpdate) return;
    const data = this.tokenize(value);
    this.setState({
      prevValue: value,
  ...
  tokenize(something) {
    ...
    if ('nodeType' in something) {
        ...
        switch (type) {
          ...
          case 'colon':
            ...
            if (followedBySymbol(i, ['}', ']'])) {
              setError(i, format(locale.invalidToken.termSequence.permitted, {
                firstTerm: locale.symbols.colon,
                secondTerm: locale.types.vOrigin
              }));
              break;
            }
      ...
      if (!error) for (var i = 0; i < buffer.tokens_merge.length; i++) {
        ...
        switch (type) {
          ...
          case 'key':
            // console.log(newLineBreakAndIndent() + newSpan(i, token, _depth))
    ...
    if (!('nodeType' in something)) {
      ...
      function add_tokenPrimary(vOrigin) {
        if (vOrigin.length === 0) return false;
        buffer.tokens.push(vOrigin);
        return true;
      }
